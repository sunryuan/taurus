...
public class CronExpression implements Serializable, Cloneable {
     ...
     * seconds are ignored, so two Dates falling on different seconds
    ...
    public boolean isSatisfiedBy(Date date) {
        ...
        testDateCal.add(Calendar.MINUTE, -1);
        
        //Date timeAfter = getTimeAfter(testDateCal.getTime());
        
        Calendar timeAfter = Calendar.getInstance(getTimeZone());
        timeAfter.setTime(getTimeAfter(testDateCal.getTime()));
        timeAfter.set(Calendar.MILLISECOND, 0);
        timeAfter.set(Calendar.SECOND, 0);
        Date nextTime = timeAfter.getTime();

        return ((timeAfter != null) && (nextTime.equals(originalDate)));
    }
